...
from songbot.env_loader import load_env_robust
...
# =========================================================
# DELAYED SEND (persist across restarts) — НОВЫЙ БЛОК
# =========================================================
...
# переопределяем глобальные, чтобы точно использовать один инстанс
DELAYED_TRACKS: Dict[str, Dict[str, Any]] = {}
DELAYED_TRACKS_LOCK = Lock()

# При старте пробуем подгрузить существующие отложенные задачи
try:
...
def _save_delayed_tracks() -> None:
    """Сохраняем DELAYED_TRACKS в delayed_tracks.json (человеческий формат)."""
    try:
...
def _schedule_delayed_send(
    ...
    now_ts = time.time()
    # Если время уже прошло — сразу шлём
